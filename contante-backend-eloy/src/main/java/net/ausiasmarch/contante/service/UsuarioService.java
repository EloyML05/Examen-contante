...
public class UsuarioService implements ServiceInterface<UsuarioEntity> {
    ...
    public Long randomCreate(Long cantidad) {
        for (int i = 0; i < cantidad; i++) {
            ...
            oUsuarioEntity.setId_tipousuario((long) oRandomService.getRandomInt(1, 3));
    ...
    public Page<UsuarioEntity> getPagePar(Pageable oPageable, Optional<String> filter) {

        if (filter.isPresent()) {
            return oUsuarioRepository.findUsuariosConIdParPorNombre(filter.get(), oPageable);
        } else {
            return oUsuarioRepository.findUsuariosConIdPar(oPageable);
        }
    }
    ...
    public UsuarioEntity duplicate(Long id) {
        UsuarioEntity oUsuarioEntityDup = oUsuarioRepository.findById(id).get();
        UsuarioEntity oUsuarioEntity = new UsuarioEntity(oUsuarioEntityDup.getNombre(),
                oUsuarioEntityDup.getApellido1(), oUsuarioEntityDup.getApellido2(), oUsuarioEntityDup.getEmail(),
                oUsuarioEntityDup.getId_tipousuario());
    ...
    public Long delete(String nombre) {
        oUsuarioRepository.deleteByNombre(nombre);
        return this.count();
    }
}